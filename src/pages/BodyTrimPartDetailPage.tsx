...
import { bodyTrimAssemblyData } from '../data/bodyTrimAssembly';
import type { BodyTrimPart } from '../data/bodyTrimAssembly';

function findBodyTrimPart(id: string): { part: BodyTrimPart; assemblyName: string; subAssemblyName: string } | null {
  for (const assembly of bodyTrimAssemblyData) {
    for (const sub of assembly.subAssemblies) {
      const part = sub.parts.find(p => p.id === id);
      if (part) return { part, assemblyName: assembly.name, subAssemblyName: sub.name };
    }
  }
  return null;
}

export default function BodyTrimPartDetailPage() {
  const { id } = useParams<{ id: string }>();
  const navigate = useNavigate();

  const result = id ? findBodyTrimPart(id) : null;
  if (!result) {
    return (
      <div className="px-4 py-8 text-center">
        <h2 className="text-2xl font-bold text-gray-900 mb-4">未找到该车身饰件零件</h2>
        <button onClick={() => navigate('/parts')} className="text-blue-600 hover:text-blue-700">返回零部件列表</button>
      </div>
    );
  }

  const { part, assemblyName, subAssemblyName } = result;
  const bodyTrimPartEdits = typeof window !== 'undefined'
    ? JSON.parse(localStorage.getItem('bodyTrimPartEdits') || '{}')
    : {};
  const customEdit = bodyTrimPartEdits[id || ''];

  const materialName = customEdit?.material || part.material;
  const materialEntries = materialName && materialName !== '-'
    ? materials.filter(m =>
  ...
  return (
    <div className="px-4 py-8 max-w-5xl mx-auto">
      <nav className="mb-6 text-sm">
        ...
        <span className="mx-2 text-gray-400">/</span>
        <span className="text-gray-500">车身饰件</span>
        <span className="mx-2 text-gray-400">/</span>
      ...
      <div className="bg-white rounded-lg shadow overflow-hidden">
        <div className="p-6 border-b border-gray-200">
          ...
          <div className="flex flex-wrap items-center gap-2 text-sm text-gray-500">
            <span className="px-2 py-0.5 bg-emerald-100 text-emerald-800 rounded">车身系统</span>
            <span className="px-2 py-0.5 bg-gray-100 text-gray-700 rounded">外观及功能饰件</span>
        ...
        <div className="h-64 bg-gradient-to-br from-emerald-50 to-green-100 flex items-center justify-center overflow-hidden border-b border-gray-200">
          {displayImage ? (
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {displayVehicleModels.map((model: string, i: number) => (
                  <span key={i} className="px-3 py-1.5 bg-amber-50 text-amber-800 text-sm rounded-lg border border-amber-200 font-medium">{model}</span>
                ))}
              </div>
            </div>
          )}
          <div>
            ...
            <div className="flex flex-wrap gap-2 mb-3">
              {materialName.split('/').map((m: string, i: number) => (
                <span key={i} className="px-3 py-1 bg-green-50 text-green-700 text-sm rounded-lg border border-green-200 font-medium">{m.trim()}</span>
              ))}
            </div>
              ...
              <div className="space-y-2">
                {materialEntries.map(mat => (
                  <Link key={mat.id} to={`/materials/${mat.id}`}
                    className="block border border-gray-200 rounded-lg p-3 hover:border-emerald-500 hover:bg-emerald-50 transition-colors">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-emerald-600 text-sm ml-3">→</span>
                    </div>
                  </Link>
                ))}
              </div>
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {displayProcess.split('/').map((p: string, i: number) => (
                <span key={i} className="px-3 py-1.5 bg-purple-50 text-purple-700 text-sm rounded-lg border border-purple-200">{p.trim()}</span>
              ))}
            </div>
          </div>
        </div>
