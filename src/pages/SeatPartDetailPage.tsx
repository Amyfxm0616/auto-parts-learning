...
import { seatAssemblyData } from '../data/seatAssembly';
import type { SeatPart } from '../data/seatAssembly';

function findSeatPart(id: string): { part: SeatPart; assemblyName: string; subAssemblyName: string } | null {
  for (const assembly of seatAssemblyData) {
...
export default function SeatPartDetailPage() {
  const { id } = useParams<{ id: string }>();
  const navigate = useNavigate();

  const result = id ? findSeatPart(id) : null;

  if (!result) {
    return (
      <div className="px-4 py-8 text-center">
        <h2 className="text-2xl font-bold text-gray-900 mb-4">未找到该座椅零件</h2>
  ...
  const seatPartEdits = typeof window !== 'undefined'
    ? JSON.parse(localStorage.getItem('seatPartEdits') || '{}')
    : {};

  const customEdit = seatPartEdits[id || ''];
  ...
  return (
    <div className="px-4 py-8 max-w-5xl mx-auto">
      ...
      <div className="bg-white rounded-lg shadow overflow-hidden">
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <div className="flex flex-wrap items-center gap-2 text-sm text-gray-500">
                <span className="px-2 py-0.5 bg-amber-100 text-amber-800 rounded">座舱系统</span>
                <span className="px-2 py-0.5 bg-gray-100 text-gray-700 rounded">座椅</span>
        ...
        <div className="h-64 bg-gradient-to-br from-amber-50 to-yellow-100 flex items-center justify-center overflow-hidden border-b border-gray-200">
          {displayImage ? (
              ...
              <div className="space-y-2">
                {materialEntries.map(mat => (
                  <Link
                    key={mat.id}
                    to={`/materials/${mat.id}`}
                    className="block border border-gray-200 rounded-lg p-3 hover:border-amber-500 hover:bg-amber-50 transition-colors"
                  >
                    <div className="flex items-center justify-between">
                      <div>
                      ...
                      <span className="text-amber-600 text-sm ml-3">→</span>
                    </div>
                  </Link>
                ))}
              </div>
